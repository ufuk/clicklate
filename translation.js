    ...
    request.onreadystatechange = function () {
        if (!completed && request.readyState == 4 && request.status == 200) {
            ...
            chrome.tabs.sendMessage(tabId, {
                method: 'showResult',
                string: getResultsForResponseText(responseText, getSearchURL(searchString), searchString)
            });
            completed = true;
        }
...
function getResultsForResponseText(responseText, url, searchString) {
    var result = "";
    ...
    console.log("New vocab is adding...");
    chrome.storage.sync.get({
        vocab: []
    }, function (results) {
        var newVocab = searchString.toLowerCase();
        if (results.vocab.indexOf(newVocab) == -1) {
            results.vocab.push({vocab: newVocab, inTurkish: [], inEnglish: []});
            chrome.storage.sync.set(results, function () {
                console.log('New vocab saved.');
            });
        }
    });
